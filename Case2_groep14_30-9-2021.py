...
st.write('Wij hebben een API gekozen over coronabesmettingen over de hele wereld en de vaccinatiegraad.')
...
st.subheader('4.1 Histogram van percentage overledenen per aantal besmette personen')


# In[35]:


st.write('We hebben een histogram gemaakt dat de waardes van de kolom percentage van de overledenen per totaalbesmette mensen laat zien. Wat wel opvalt is dat de histogram een normale verdeling lijkt te volgen. Verder zien we dat veel mensen die besmet raakte een best wel hoge kans heeft om de ziekte te overleven(>97%).')
...
fig1 = px.histogram(data_frame=df, x='%_deaths',
                   title = 'Histogram van percentage overledenen per aantal besmette personen',
...
fig3.update_layout(title="Levensverwachting per land",
                  xaxis_title='Landen per continent',
